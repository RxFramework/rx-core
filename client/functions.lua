...
function RXCore.Functions.CreateClientCallback(name, cb)
    RXCore.ClientCallbacks[name] = cb
end

function RXCore.Functions.TriggerCallback(name, ...)
    local cb = nil
    local args = { ... }
    ...
    RXCore.ServerCallbacks[name] = {
        callback = cb,
        promise = promise.new()
    }

    TriggerServerEvent('RXCore:Server:TriggerCallback', name, table.unpack(args))

    if cb == nil then
        Citizen.Await(RXCore.ServerCallbacks[name].promise)
        return RXCore.ServerCallbacks[name].promise.value
    end
end

function RXCore.Debug(resource, obj, depth)
    TriggerServerEvent('RXCore:DebugSomething', resource, obj, depth)
end

-- Player

function RXCore.Functions.GetPlayerData(cb)
    if not cb then return RXCore.PlayerData end
    cb(RXCore.PlayerData)
end
...
function RXCore.Functions.HasItem(items, amount)
    return exports['rx-inventory']:HasItem(items, amount)
end

---Returns the full character name
---@return string
function RXCore.Functions.GetName()
    local charinfo = RXCore.PlayerData.charinfo
...
function RXCore.Functions.LookAtEntity(entity, timeout, speed)
...
function RXCore.Functions.PlayAnim(animDict, animName, upperbodyOnly, duration)
...
function RXCore.Functions.IsWearingGloves()
    local ped = PlayerPedId()
    local armIndex = GetPedDrawableVariation(ped, 3)
    local model = GetEntityModel(ped)
    if model == `mp_m_freemode_01` then
        if RXCore.Shared.MaleNoGloves[armIndex] then
            return false
        end
    else
        if RXCore.Shared.FemaleNoGloves[armIndex] then
            return false
        end
    end
    return true
...
function RXCore.Functions.Notify(text, texttype, length, icon)
    local message = {
        action = 'notify',
...
function RXCore.Functions.Progressbar(name, label, duration, useWhileDead, canCancel, disableControls, animation, prop, propTwo, onFinish, onCancel)
    if GetResourceState('progressbar') ~= 'started' then error('progressbar needs to be started in order for RXCore.Functions.Progressbar to work') end
...
function RXCore.Functions.GetVehicles()
    return GetGamePool('CVehicle')
end

function RXCore.Functions.GetObjects()
...
function RXCore.Functions.GetPlayersFromCoords(coords, distance)
...
function RXCore.Functions.GetClosestPlayer(coords)
    local ped = PlayerPedId()
    if coords then
        coords = type(coords) == 'table' and vec3(coords.x, coords.y, coords.z) or coords
    else
    ...
    local closestPlayers = RXCore.Functions.GetPlayersFromCoords(coords)
...
function RXCore.Functions.GetPeds(ignoreList)
...
function RXCore.Functions.GetClosestPed(coords, ignoreList)
    local ped = PlayerPedId()
    if coords then
        coords = type(coords) == 'table' and vec3(coords.x, coords.y, coords.z) or coords
    else
    ...
    ignoreList = ignoreList or {}
    local peds = RXCore.Functions.GetPeds(ignoreList)
    local closestDistance = -1
    local closestPed = -1
    for i = 1, #peds, 1 do
...
function RXCore.Functions.GetClosestVehicle(coords)
    local ped = PlayerPedId()
...
function RXCore.Functions.GetClosestObject(coords)
    local ped = PlayerPedId()
...
function RXCore.Functions.LoadModel(model)
...
function RXCore.Functions.SpawnVehicle(model, cb, coords, isnetworked, teleportInto)
    local ped = PlayerPedId()
    ...
    RXCore.Functions.LoadModel(model)
...
function RXCore.Functions.DeleteVehicle(vehicle)
    SetEntityAsMissionEntity(vehicle, true, true)
    DeleteVehicle(vehicle)
end

function RXCore.Functions.GetPlate(vehicle)
    if vehicle == 0 then return end
    return RXCore.Shared.Trim(GetVehicleNumberPlateText(vehicle))
end

function RXCore.Functions.GetVehicleLabel(vehicle)
    if vehicle == nil or vehicle == 0 then return end
    return GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(vehicle)))
end

function RXCore.Functions.GetVehicleProperties(vehicle)
    if DoesEntityExist(vehicle) then
        ...
        return {
            ...
            plate = RXCore.Functions.GetPlate(vehicle),
            plateIndex = GetVehicleNumberPlateTextIndex(vehicle),
            bodyHealth = RXCore.Shared.Round(GetVehicleBodyHealth(vehicle), 0.1),
            engineHealth = RXCore.Shared.Round(GetVehicleEngineHealth(vehicle), 0.1),
            tankHealth = RXCore.Shared.Round(GetVehiclePetrolTankHealth(vehicle), 0.1),
            fuelLevel = RXCore.Shared.Round(GetVehicleFuelLevel(vehicle), 0.1),
            dirtLevel = RXCore.Shared.Round(GetVehicleDirtLevel(vehicle), 0.1),
            oilLevel = RXCore.Shared.Round(GetVehicleOilLevel(vehicle), 0.1),
            color1 = colorPrimary,
...
function RXCore.Functions.SetVehicleProperties(vehicle, props)
    if DoesEntityExist(vehicle) then
...
function RXCore.Functions.DrawText(x, y, width, height, scale, r, g, b, a, text)
...
function RXCore.Functions.DrawText3D(x, y, z, text)
    exports['rx-core']:DrawText3D(x, y, z, text)
end

function RXCore.Functions.DrawText3Ds(...)
    exports['rx-core']:DrawText3Ds(...)
end

function RXCore.Functions.ShowTextUI(text, position)
    exports['rx-core']:ShowTextUI(text, position)
end

function RXCore.Functions.HideTextUI()
    exports['rx-core']:HideTextUI()
end

function RXCore.Functions.RequestAnimDict(animDict)
...
function RXCore.Functions.GetClosestBone(entity, list)
...
function RXCore.Functions.GetBoneDistance(entity, boneType, boneIndex)
    local bone
    if boneType == 1 then
...
function RXCore.Functions.AttachProp(ped, model, boneId, x, y, z, xR, yR, zR, vertex)
    local modelHash = type(model) == 'string' and joaat(model) or model
    local bone = GetPedBoneIndex(ped, boneId)
    RXCore.Functions.LoadModel(modelHash)
...
function RXCore.Functions.SpawnClear(coords, radius)
    if coords then
        coords = type(coords) == 'table' and vec3(coords.x, coords.y, coords.z) or coords
    else
...
function RXCore.Functions.LoadAnimSet(animSet)
...
function RXCore.Functions.LoadParticleDictionary(dictionary)
...
function RXCore.Functions.StartParticleAtCoord(dict, ptName, looped, coords, rot, scale, alpha, color, duration)
    if coords then
        coords = type(coords) == 'table' and vec3(coords.x, coords.y, coords.z) or coords
    else
        coords = GetEntityCoords(PlayerPedId())
    end
    RXCore.Functions.LoadParticleDictionary(dict)
...
function RXCore.Functions.StartParticleOnEntity(dict, ptName, looped, entity, bone, offset, rot, scale, alpha, color, evolution, duration)
    RXCore.Functions.LoadParticleDictionary(dict)
    UseParticleFxAssetNextCall(dict)
...
function RXCore.Functions.GetStreetNametAtCoords(coords)
...
function RXCore.Functions.GetZoneAtCoords(coords)
    return GetLabelText(GetNameOfZone(coords))
end

function RXCore.Functions.GetCardinalDirection(entity)
...
function RXCore.Functions.GetCurrentTime()
    local obj = {}
...
function RXCore.Functions.GetGroundZCoord(coords)
...
function RXCore.Functions.GetGroundHash(entity)
    local coords = GetEntityCoords(entity)
...
-- exports['rx-core']:Notify('Hello Player!')
